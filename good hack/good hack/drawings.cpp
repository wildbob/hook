...
{
	static auto& w2sMatrix = g_csgo.m_engine->world_to_screen_matrix();
...
void c_drawings::setup_fonts()
{
	if (!font)
	{
		g_csgo.m_surface->set_font_glyph(font1 = g_csgo.m_surface->create_font(), "Tahoma", 12, FW_DONTCARE, NULL, NULL, (int)FontFlags::FONTFLAG_ANTIALIAS | (int)FontFlags::FONTFLAG_OUTLINE);
		font = true;
	}
}
...
void c_drawings::draw_string(HFont font, int x, int y, Color color, DWORD alignment, const char* msg, ...)
{
	...
	g_csgo.m_surface->get_text_size(font, formated, iWidth, iHeight);

	if (alignment & FONT_CENTER)
		x -= iWidth / 2;

	g_csgo.m_surface->draw_set_text_pos(x, y);
	g_csgo.m_surface->draw_set_text_color(color);
	g_csgo.m_surface->draw_set_font(font);
	g_csgo.m_surface->draw_print_text(formated, wcslen(formated));
}
...
void c_drawings::draw_string_unicode(HFont font, int x, int y, Color color, bool bCenter, const wchar_t* msg, ...)
{
	...
	g_csgo.m_surface->get_text_size(font, msg, iWidth, iHeight);
	g_csgo.m_surface->draw_set_font(font);
	g_csgo.m_surface->draw_set_text_color(Color(r, g, b, a));
	g_csgo.m_surface->draw_set_text_pos(!bCenter ? x : x - iWidth / 2, y - iHeight / 2);
	g_csgo.m_surface->draw_print_text(msg, wcslen(msg));
}

void c_drawings::draw_rect(int x, int y, int w, int h, Color col)
{
	g_csgo.m_surface->draw_set_color(col);
	g_csgo.m_surface->draw_filled_rect(x, y, x + w, y + h);
}
...
{
	g_csgo.m_surface->draw_set_color(col);
	g_csgo.m_surface->draw_filled_rect(x, y, x + 1, y + 1);
}
...
{
	g_csgo.m_surface->draw_set_color(col);
	g_csgo.m_surface->draw_outlined_circle(x, y, r, 1);
}

void c_drawings::draw_line(int x0, int y0, int x1, int y1, Color col)
{
	g_csgo.m_surface->draw_set_color(col);
	g_csgo.m_surface->draw_line(x0, y0, x1, y1);
}
...
int GetStringWidth(HFont font, const char* msg, ...)
{
	...
	g_csgo.m_surface->get_text_size(font, wbuf, iWidth, iHeight);
